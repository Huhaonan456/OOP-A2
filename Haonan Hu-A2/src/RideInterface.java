...
public interface RideInterface {
     ...
     * 信息内容可包含游客的姓名、年龄、性别、门票类型等
     */
    void printQueue();

    /**
     * 运行一次游乐设施循环的方法。
     * 此方法负责执行一次游乐设施完整的运行操作流程，具体包括：
     ...
     * @param visitor 要添加到乘坐历史记录的游客对象，不能为空，否则可能无法正确添加。
     */
    void addVisitorToHistory(Visitor visitor);

    /**
     * 检查游客是否在乘坐历史记录中的方法。
     * 接收一个游客对象作为参数，通过逻辑判断该游客是否已经存在于乘坐历史记录里，
     * 最终返回一个布尔值来表示判断结果，true表示游客在乘坐历史记录中，false表示不在。
